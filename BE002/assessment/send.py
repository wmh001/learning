...
k=0.29
while True:
    yolo_dir = 'D:\yolov\yolov3'  # YOLO文件路径一定不能有中文在路径里面
...
# 加载网络、配置权重
    ...
    blobImg = cv2.dnn.blobFromImage(
        img, 1.0 / 255.0, (416, 416), None, True,
    ...
    idxs = cv2.dnn.NMSBoxes(boxes, confidences, CONFIDENCE,
                            THRESHOLD)  # boxes中，保留的box的索引index存入idxs
    # 应用检测结果
    np.random.seed(42)
        ...
        for i in idxs.flatten():  # indxs是二维的，第0维是输出层，所以这里把它展平成1维
            if labels[classIDs[i]] == 'person':
                ...
                cv2.putText(img, text, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        color, 2)  # cv.FONT_HERSHEY_SIMPLEX字体风格、0.5字体大小、粗细2px
                xc = int(x + w/2)
                yc = int(y + h/2)
                cv2.circle(img, (xc, yc), 20, (0, 0, 255), 5)
                # 以识别框中点为圆心绘制一个圆
                # 图片中点为（310，220）
                cv2.circle(img, (310, 220), 0, (0, 0, 255), 5)
                break
    end = time.time()
    print("YOLO1 took {:.6f} seconds".format(end - start))
    cv2.imshow('target detect result1', img)
    myangle=int(90+k*(xc-310))
    print(myangle)
    ser.write((str(myangle)+" 45 90 45 ").encode())
    time.sleep(1)
    cv2.waitKey(100)
